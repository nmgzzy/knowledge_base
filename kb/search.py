...
def search_kb(
    ...
    *,
    ...
    logger.info("search mode=%s top=%d fts_k=%d vec_k=%d", mode, int(top_k), int(fts_k), int(vec_k))

    conn = open_db(db_path)
    try:
        fts_hits = search_fts(conn, query=query, limit=fts_k)
        logger.info("fts hits=%d", len(fts_hits))
        ...
        if semantic or hybrid:
            ...
            logger.info("vec hits=%d", len(vec_scores))
...
def _semantic_scores(conn, oa_cfg, *, query: str, top_k: int) -> dict[str, float]:
    ...
    total = conn.execute("SELECT COUNT(1) AS n FROM embeddings WHERE model=?", (oa_cfg.model_embed,)).fetchone()
    n_total = int(total["n"] or 0) if total is not None else 0
    logger.info("semantic scan embeddings=%d (full table scan)", n_total)

    heap: list[tuple[float, str]] = []
    scanned = 0
    for chunk_id, dim, blob, norm in iter_embeddings(conn, model=oa_cfg.model_embed):
        scanned += 1
        if logger.isEnabledFor(logging.DEBUG) and scanned % 5000 == 0 and n_total > 0:
            logger.debug("semantic scan progress %d/%d", scanned, n_total)
        if dim <= 0 or norm <= 0:
            continue
    ...
    logger.info("semantic scan done top=%d elapsed_ms=%d", len(best), elapsed_ms)
